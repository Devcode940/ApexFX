...
export const AiAssistant: React.FC<AiAssistantProps> = () => {
  ...
  const handleSendMessage = async (textToSend = inputText, imageToSend: string | null = attachedImage) => {
    ...
    try {
      ...
      // No canned/offline fallback: surface the real reason the live AI service failed.
      const reason = err?.message || 'The AI service is unavailable. Check that GEMINI_API_KEY is configured.';
      const aiMsgTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      setMessages((prev) => [
        ...prev,
        {
          sender: 'ai',
          text: `⚠️ **Live AI service error**: ${reason}`,
          time: aiMsgTime,
        }
      ]);
    } finally {
