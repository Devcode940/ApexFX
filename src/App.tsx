...
function TradingTerminal() {
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-zinc-950 text-zinc-100' : 'bg-zinc-50 text-zinc-900'} font-sans flex flex-col selection:bg-emerald-600/30 ${isChartFullScreen ? 'h-screen w-screen overflow-hidden' : ''}`}>
      
      {/* 1. TOP GLOBAL INSTRUMENT BAR / HUD HEADER (Hidden in Fullscreen Focus Mode) */}
      {!isChartFullScreen && (
      ...
      <main className={`flex-1 overflow-hidden flex flex-col md:flex-row ${isChartFullScreen ? 'h-screen w-screen' : ''}`}>
        
        {/* Left Side: Live FX Watchlist (Hidden in Fullscreen Focus Mode) */}
        {!isChartFullScreen && (
          leftSidebarOpen ? (
        ...
        <section className={`flex-1 bg-zinc-950 flex flex-col ${
          isChartFullScreen 
            ? 'p-1.5 sm:p-2 h-screen w-screen overflow-hidden' 
            : `border-r border-zinc-800 overflow-y-auto p-4 space-y-4 ${mobileTab !== 'chart' ? 'hidden md:flex' : 'flex'}`
        }`}>
          
          {/* Controls Bar: Timeframes, Indicators toggles (Hidden in Fullscreen Focus Mode) */}
          {!isChartFullScreen && (
          ...
          {activeData.length > 0 ? (
            ...
            <div className={`w-full ${isChartFullScreen ? 'h-full flex-1' : 'h-[620px]'} rounded-lg border border-zinc-800 bg-zinc-950 flex items-center justify-center text-zinc-500 font-mono text-xs`}>
              Initializing Forex charting buffer...
            </div>
          )}

          {/* Mobile responsive tab buttons (Hidden in Fullscreen Focus Mode) */}
          {!isChartFullScreen && (
          ...
          {/* Institutional Macro, Currency Strength & ForexFactory Economic Calendar Deck (Hidden in Fullscreen Focus Mode) */}
          {!isChartFullScreen && (
          ...
          {/* Bottom Grid for Secondary Panels (Hidden in Fullscreen Focus Mode) */}
          {!isChartFullScreen && (
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          {/* Comprehensive Performance & Telemetry Dashboard (Hidden in Fullscreen Focus Mode) */}
          {!isChartFullScreen && (
        ...
        {/* Right Side: Execution & Trade Signal Analysis Deck (Hidden in Fullscreen Focus Mode) */}
        {!isChartFullScreen && (
          rightSidebarOpen ? (
        ...
        {/* Mobile Tab Overlay displays (Hidden in Fullscreen Focus Mode) */}
        {!isChartFullScreen && (
