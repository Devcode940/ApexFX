...
app.use((req, _res, next) => {
  ...
  const reqId = typeof incoming === 'string' && incoming.length > 0 && incoming.length < 64
    ? incoming
...
// ============================================================
// Live streaming + WS — constructed lazily in startServer()
// so that importing `app` for serverless (Vercel) is side-effect
// free (no open sockets, no background timers).
// ============================================================
...
let tdWsCleanupTokensInterval: NodeJS.Timeout | null = null;
let tdWsLastPriceAt = 0;
let tdSyncInFlight = false;
let quoteSyncInterval: NodeJS.Timeout | null = null;
let pollWatchdogInterval: NodeJS.Timeout | null = null;
let yahooPollInterval: NodeJS.Timeout | null = null;
...
function broadcastPrices() {
  ...
  const BUFFER_HIGH_WATER_MARK = 512 * 1024; // 512 KB
  wss.clients.forEach((client) => {
    if (client.readyState !== WebSocket.OPEN) return;
    try {
      // Drop slow consumers to prevent memory exhaustion
      if ((client as any).bufferedAmount > BUFFER_HIGH_WATER_MARK) {
        warn('[WS] terminating slow consumer; buffer=' + (client as any).bufferedAmount);
        client.terminate();
        return;
      }
      client.send(payload);
...
function startTwelveDataStream() {
  ...
  try {
    tdWs = new WebSocket(`wss://ws.twelvedata.com/v1/quotes/price?apikey=${tdApiKey}`, {
      headers: { 'User-Agent': 'ApexFX-Terminal/1.0 (Production)' },
    } as any);
    tdWs.on('open', () => {
      ...
      tdWsHeartbeatTimer = setInterval(() => {
        try { safeSend(tdWs, { action: 'heartbeat' }); } catch { /* ignore */ }
      }, 10_000);
    });
    tdWs.on('message', (data) => {
      try {
        ...
        if (msg.event === 'price' && msg.symbol && msg.price) {
          ...
          if (msg.status === 'error' || msg.status === 'unsubscribed') {
            warn('[TwelveData] subscription issue:', msg.message || JSON.stringify(msg).slice(0, 200));
          }
        } else if (msg.status === 'error') {
          warn('[TwelveData] WS error message:', msg.message || JSON.stringify(msg).slice(0, 200));
        }
    ...
    warn('[TwelveData] WebSocket setup failed:', (_e as Error).message);
    tdWs = null;
    scheduleTdWsReconnect();
  }
}
...
function startBackgroundFeeds() {
  // Stop anything that was previously running (defensive)
  stopBackgroundFeeds();

  // Cleanup expired WS tokens every minute
  tdWsCleanupTokensInterval = setInterval(() => {
    const now = Date.now();
    for (const [token, data] of wsAuthTokens) {
      if (now - data.createdAt > WS_TOKEN_EXPIRY_MS) wsAuthTokens.delete(token);
    }
  }, 60_000);
...
function stopBackgroundFeeds() {
  ...
  if (quoteSyncInterval) { clearInterval(quoteSyncInterval); quoteSyncInterval = null; }
  if (pollWatchdogInterval) { clearInterval(pollWatchdogInterval); pollWatchdogInterval = null; }
  if (yahooPollInterval) { clearInterval(yahooPollInterval); yahooPollInterval = null; }
  if (tdWs) { try { tdWs.close(); } catch { /* ignore */ } tdWs = null; }
}

function attachWebSocketServer(httpServer: HttpServer) {
  wss = new WebSocketServer({
    server: httpServer,
    verifyClient: (info, callback) => {
      const origin = info.origin || info.req.headers.origin;
      if (process.env.NODE_ENV === 'production') {
        if (!origin) { callback(true); return; }
        if (ALLOWED_ORIGINS.includes(origin)) callback(true);
        else callback(false, 403, 'Unauthorized origin');
      } else {
  ...
  wss.on('connection', (ws, req) => {
    ...
    const connectingIp = sanitizeClientIp(req);
    if (!tokenData) {
      ws.send(JSON.stringify({ type: 'ERROR', message: 'Authentication failed' }));
      ws.close(4001, 'Unauthorized');
      return;
    }
    if (Date.now() - tokenData.createdAt > WS_TOKEN_EXPIRY_MS) {
      wsAuthTokens.delete(token);
      ws.send(JSON.stringify({ type: 'ERROR', message: 'Token expired' }));
      ws.close(4002, 'Token Expired');
      return;
    }
    if (tokenData.ip !== connectingIp) {
      wsAuthTokens.delete(token);
      ws.send(JSON.stringify({ type: 'ERROR', message: 'Token not valid for this client' }));
      ws.close(4003, 'Token IP mismatch');
      return;
    }
...
app.get('/api/ws/token', async (req, res) => {
  ...
  if (requiredSecret) {
    const provided = req.headers['x-ws-secret'] as string | undefined;
...
async function generateOpenRouter(systemPrompt: string, contents: any[]) {
  const messages = [{ role: 'system', content: systemPrompt }, ...toOpenRouterMessages(contents)];
...
app.get('/api/forex', async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Frankfurter API returned status ${response.status}`);
    ...
    const result = {
      ...
      timestamp: data.date,
      rates: { EURUSD: eurRates, GBPUSD: gbpRates, USDJPY: jpyRates, AUDUSD: audRates, USDCAD: cadRates, GBPJPY: gbpjpyRates },
    };
...
app.get('/api/market/prices', async (req, res) => {
  ...
  try {
    ...
    const cached = priceCache.get(cacheKey);
    if (cached && Date.now() - lastPriceFetchTs < PRICE_FETCH_CACHE_MS) return res.json(cached);
    ...
    const result = {
      success: true,
      rates: serverWatchlist.reduce((acc, item) => {
        acc[item.symbol] = { price: item.price, high: item.high, low: item.low, change: item.change };
        return acc;
      }, {} as Record<string, any>),
      timestamp: new Date().toISOString(),
    };
...
app.get('/api/market/history', async (req, res) => {
  ...
  try {
    ...
    if (!symbol || !timeframe) return res.status(400).json({ error: 'Symbol and timeframe are required' });

    const sym = String(symbol).toUpperCase();
    const tf = String(timeframe);

    if (!validateSymbolFormat(sym, false)) return res.status(400).json({ error: 'Invalid symbol format' });

    const validTimeframes = ['1m', '5m', '15m', '1H', '4H', 'D'];
    if (!validTimeframes.includes(tf)) return res.status(400).json({ error: 'Invalid timeframe' });
    if (sym.length > 10) return res.status(400).json({ error: 'Symbol too long' });
    ...
    // fetchMarketHistory already populates historyCache for yahoo responses;
    // ensure canonical key is populated too.
...
app.get('/api/market/quote', async (req, res) => {
  ...
  try {
    ...
    if (!validateSymbolFormat(symbol as string, true)) return res.status(400).json({ error: 'Invalid symbol format' });
    const response = await fetchWithTimeout(
...
// Vite middleware or production serving + start
